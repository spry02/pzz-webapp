## @file app.py
## @brief Główny moduł aplikacji Flask obsługujący żądania HTTP i logikę biznesową
## @details Zawiera definicje tras (routes), kontrolery i funkcje pomocnicze aplikacji
...
## @brief Główna klasa aplikacji Flask
class StockMarketApp:
    ## @brief Inicjalizacja aplikacji Flask i konfiguracja
    def __init__(self):
        ...
        self.app.config['USE_RELOADER'] = True
        ...
        # Dodanie dekoratora before_request
        @self.app.before_request
        def redirect_to_http():
            """Redirect HTTPS requests to HTTP"""
            # @brief Przekierowuje żądania HTTPS na HTTP
            # @return Przekierowanie lub None jeśli nie jest to HTTPS
            if request.headers.get('X-Forwarded-Proto') == 'https':
                url = request.url.replace('https://', 'http://', 1)
                return redirect(url, code=301)

    ## @brief Rejestruje wszystkie endpointy aplikacji
    def _register_endpoints(self):
        # Główne trasy
        self.app.route('/')(self.login_required(self.index))
        self.app.route('/dashboard')(self.login_required(self.dashboard))
        self.app.route('/admin_panel', methods=['GET', 'POST'])(self.login_required(self.admin_panel))
        self.app.route('/edit_profile', methods=['GET', 'POST'])(self.login_required(self.edit_profile))
        self.app.route('/register', methods=['GET', 'POST'])(self.register)
        self.app.route('/login', methods=['GET', 'POST'])(self.login)
        self.app.route('/logout')(self.logout)
        
        # API endpoints
        self.app.route('/api/market_data')(self.login_required(self.api_market_data))
        self.app.route('/api/open_positions')(self.login_required(self.api_open_positions))
        self.app.route('/api/close_position', methods=['POST'])(self.login_required(self.api_close_position))
        self.app.route('/api/add_funds', methods=['POST'])(self.login_required(self.add_funds))
        self.app.route('/api/transaction', methods=['POST'])(self.login_required(self.create_transaction))

    ## @brief Dekorator sprawdzający, czy użytkownik jest zalogowany
    ## @param f Funkcja, która ma być dekorowana
    ## @return Dekorowana funkcja, która albo uruchamia się normalnie albo przekierowuje na stronę logowania
    ...
    ## @brief Przekierowuje na stronę główną (dashboard) dla zalogowanych użytkowników
    ## @return Przekierowanie na odpowiednią stronę 
    def index(self):
        return redirect(url_for('dashboard'))

    ## @brief Pobiera historyczne dane dla instrumentów finansowych
    ## @param symbol Symbol instrumentu finansowego
    ## @param days_back Liczba dni, dla których mają być wygenerowane dane (opcjonalnie)
    ## @return Tuple zawierająca listę cen i listę dat
    def get_historical_data(self, symbol, days_back=None):
    ...
    ## @brief Pobiera symulowane dane rynkowe dla zewnętrznych instrumentów finansowych
    ## @return Słownik zawierający dane rynkowe wraz ze wskaźnikami
    def get_external_market_data(self):
        try:
            ...
            instruments = [
                {
                    'symbol': 'SI=F', 
                    'name': 'Srebro', 
                    'prices': self.get_historical_data('SILVER', 60)[0],
                    'dates': self.get_historical_data('SILVER', 60)[1]
                },
                {
                    'symbol': 'GC=F', 
                    'name': 'Złoto',
                    'prices': self.get_historical_data('GOLD', 60)[0],
                    'dates': self.get_historical_data('GOLD', 60)[1]
                },
                {
                    'symbol': 'BTC-USD', 
                    'name': 'Bitcoin',
                    'prices': self.get_historical_data('BTC', 60)[0],
                    'dates': self.get_historical_data('BTC', 60)[1]
                },
                {
                    'symbol': 'NG=F', 
                    'name': 'Gaz Ziemny',
                    'prices': self.get_historical_data('NATGAS', 60)[0],
                    'dates': self.get_historical_data('NATGAS', 60)[1]
                },
                {
                    'symbol': 'AAPL', 
                    'name': 'Apple Inc.',
                    'prices': self.get_historical_data('AAPL', 60)[0],
                    'dates': self.get_historical_data('AAPL', 60)[1]
                }
            ]
            ...
            wig20_prices, wig20_dates = self.get_historical_data('WIG20', 30)
            ...
            self.app.logger.error(f"Error fetching market data: {str(e)}")
            return {
                'market_indicators': {
    ...
    ## @brief Pobiera rzeczywiste dane rynkowe z Yahoo Finance
    ## @return Słownik zawierający dane rynkowe wraz ze wskaźnikami
    ...
    ## @brief Wyświetla dashboard z wykresami instrumentów handlowych dla zalogowanego użytkownika
    ## @details Pobiera saldo użytkownika z bazy danych i renderuje stronę dashboard
    ## @return Strona dashboard z danymi użytkownika i instrumentami
    def dashboard(self):
    ...
    ## @brief Endpoint API dla pobierania danych rynkowych
    ## @return Dane rynkowe w formacie JSON
    def api_market_data(self):
        return jsonify(self.get_real_market_data())

    ## @brief Endpoint API zwracający otwarte pozycje użytkownika
    ## @return Lista otwartych pozycji w formacie JSON
    def api_open_positions(self):
    ...
    ## @brief Endpoint API do zamykania pozycji
    ## @return Status operacji w formacie JSON
    def api_close_position(self):
        data = request.json
        user_id = session['user_id']
        symbol = data['symbol']
        ...
        transaction_id = self.db.create_transaction(user_id, instrument_id, transaction_type, amount, price)
    ...
    ## @brief Endpoint API do dodawania środków do konta użytkownika
    ## @return Status operacji w formacie JSON
    def add_funds(self):
        data = request.json
    ...
    ## @brief Wyświetla panel administratora
    ## @return Strona panelu administratora z listą użytkowników i transakcji
    def admin_panel(self):
        ...
        if request.method == 'POST' and 'action' in request.form:
            ...
            if action == 'delete_user':
                ...
                if self.db.update_user_role(user_id, user_role):
    ...
    ## @brief Obsługuje edycję profilu użytkownika
    ## @return Strona edycji profilu lub odpowiedź JSON z wynikiem operacji
    def edit_profile(self):
        self.app.logger.debug(f"Edytuj profil ścieżka wywołana metodą: {request.method}")
        if request.method=='POST' and 'typ' in request.form:
            typ = request.form.get('typ')
            if typ == 'usernameEdit':
                email = request.form.get('username')
                if self.db.update_user_email(session['email'], email):
                    session['email'] = email
                ...
                if self.db.update_user_password(session['email'], passwordOld, passwordNew):
                ...
                if self.db.delete_user(session['email']):
    ...
    ## @return Strona rejestracji lub odpowiedź JSON z wynikiem operacji
    def register(self):
        self.app.logger.debug(f"Zarejestruj ścieżkę wywołaną metodą: {request.method}")
    ...
    ## @return Strona logowania lub odpowiedź JSON z wynikiem operacji
    def login(self):
        if request.method == 'POST':
            email = request.form.get('username')
            password = request.form.get('password')
    ...
    ## @details Czyści dane sesji i przekierowuje na stronę logowania
    ...
    ## @brief Endpoint API do tworzenia transakcji
    ## @return Status operacji w formacie JSON
    ...
    ## @brief Uruchamia aplikację Flask
    ## @param host Adres hosta
    ## @param port Numer portu
    ## @param debug Tryb debugowania
    ## @param use_reloader Czy używać automatycznego przeładowywania
    def run(self, host='0.0.0.0', port=5000, debug=True, use_reloader=False):
...
if __name__ == '__main__':
    ...
    stock_app.run(host='0.0.0.0', port=5000, debug=True, use_reloader=False)